...
def embedd_documents(embedding_queue: Queue):
    while True:
        document_obj: DocumentModel = embedding_queue.get()
        print(document_obj.file.path)
        ...
        doc_list.extend(give_prompt_from_document(file_path=file_path))

        texts.extend(doc_list[0])

        for text in texts:
            text.metadata = {
                'page': text.metadata['page'], 'source': document_obj.file.path, 'name': document_obj.name}
        print(texts)

        vector_db.add_documents(texts)
        vector_db.persist()

        document_obj.embeddingStatus = DocumentModel.COMPLETED
        document_obj.save()
        print(f'Doc : {document_obj.embeddingStatus}')
...
def give_prompt_from_document(file_path):
    ...
    filtered_tables, filtered_pages = filter_table(
